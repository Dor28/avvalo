...
def test_required_prompt_files_exist() -> None:
    for name in ("system_safety.txt", "family_shield.txt", "seller_guard.txt"):
        _read(name)
...
@pytest.mark.parametrize("name", ["family_shield.txt", "seller_guard.txt"])
...
def test_seller_guard_prompt_forbids_claiming_money_arrived() -> None:
    # §8: "(Seller Guard) NEVER state that money arrived/was received".
    assert "money has arrived" in _read("seller_guard.txt").lower()
