...
| Allowlisted semantic router | ✅ Implemented and tested | `OpenAICompatibleKnowledgeRouter` sees minimized text plus a server allowlist only; backend validation rejects invented IDs. Tests cover inflected Russian recall, timeout/failure degradation, default-off config, and token-cost aggregation |
