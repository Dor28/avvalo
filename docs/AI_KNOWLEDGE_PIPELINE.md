...
| Intake, language, and OCR | ✅ Implemented and tested | Text/image intake, language resolution, OCR abstraction, confidence gating, and metadata stripping exist. The default `OCR_PROVIDER=rapidocr` runs PP-OCRv5 locally on ONNX Runtime with weights baked into the image, so submitted images no longer leave the VM by default; `test_rapidocr_is_the_default_provider` guards the default and Cloud Vision remains opt-in |
...
| Reviewed cases | ⚠️ Contract present; no intake pipeline | Cards and events carry validated `reviewed_case_ids`, but current approved cards reference no reviewed derivatives. The retired `story_submission` table was dropped in migration `0013`, so no story rows exist at all. Founder-authored public posts are an editorial surface, not reviewed-case grounding, and are never injected into answers |
...
| Privacy-safe persistence | ✅ Implemented and tested | Active check, router, and URL-reputation paths persist only IDs, enums, hashes, versions, and metrics. No table carries a content column: `story_submission` and its `minimized_text` were dropped in migration `0013` |
