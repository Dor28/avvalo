...
> **Decision context:** Founder chose **"build the shared engine, demo both faces"** (Family Shield + Seller Guard), targeting **IT Park / grants**. See the strategic discussion that led here and the critique in [V1_MVP_PRODUCT_REVIEW.md](V1_MVP_PRODUCT_REVIEW.md).
> **Authority:** Vision & safety rules from [PRODUCT_GUIDE.md](PRODUCT_GUIDE.md) still win on any conflict. Engine details reuse [FAMILY_SHIELD_VALIDATION.md](FAMILY_SHIELD_VALIDATION.md).
...
> Build the **engine to production quality once.** Build **Family Shield as the polished demo.** Build **Seller Guard as a thin second skin** — enough to demo the same engine on a merchant problem, *not* a real merchant business (no subscriptions, no team accounts, no payment-provider integration in this build).
...
- **Pluggable rule layer**: one rule pack per face (Family Shield = 5 consumer scam families; Seller Guard = ~5 merchant families).
...
| Layer | Build once (shared asset) | Family Shield face | Seller Guard face (the only delta) |
|---|---|---|---|
...
- **A small but real number.** A 30–60 person Family Shield mini-alpha + a handful of merchants saying "I'd use this" beats zero. For IT Park these are supporting evidence, not a gate.
