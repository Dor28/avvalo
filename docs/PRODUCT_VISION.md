...
**What is actually built (verified against the code today):** the full check pipeline (intake → OCR → PII minimization → deterministic rules → LLM → safety validator → fixed 🚩/✅/❓ output), a working Telegram bot (@Avvalo_official_bot) with consent/privacy/deletion flows, an anonymous web app with a public landing page, a consumer checker at `/check`, and a direct-only merchant checker at `/merchants`, 7 consumer + 5 merchant rule families, three language forms (UZ-Latin, UZ-Cyrillic, RU), OCR via Cloud Vision / Tesseract / PaddleOCR, per-user daily limits, retention TTLs, a metrics export, Docker prod stack, CI, and ~27 test modules. **Not built:** payments, accounts, any person-lookup, any content/community surface, alerts.
