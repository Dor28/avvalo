...
- [ ] **A5. Web channel:** if `WEB_ENABLED=true` — landing page works on `/`; anonymous text checks work on `/check` and the direct-only `/merchants` endpoint; consent enforced before processing; Turnstile fires on image upload; over-limit path returns the friendly message.
